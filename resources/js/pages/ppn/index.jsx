                ...
                <Table>
                    <TableHeader>
                        <TableRow>
                            ...
                            <TableHead onClick={() => handleSort('ppn_name')}>
                                <SortIndicator label='Name' column='ppn_name' field={params?.field} direction={params?.direction} />
                            </TableHead>
                            <TableHead onClick={() => handleSort('ppn_nominal')}>
                                <SortIndicator label='Nominal' column='ppn_nominal' field={params?.field} direction={params?.direction} />
                            </TableHead>
                            ...
                            <>
                                {ppns.map((ppn, i) => (
                                    <TableRow key={i}>
                                        ...
                                        <TableCell>{ppn.ppn_name}</TableCell>
                                        <TableCell>{ppn.ppn_nominal}</TableCell>
                                        <TableCell>{ppn.status == 'Aktif' ? <span class='me-2 rounded bg-green-100 px-2.5 py-0.5 text-xs font-medium text-green-800 dark:bg-green-900 dark:text-green-300'>{ppn.status}</span> : <span class='me-2 rounded bg-red-100 px-2.5 py-0.5 text-xs font-medium text-red-800 dark:bg-red-900 dark:text-red-300'>{ppn.status}</span>}</TableCell>
                                        <TableCell>
                                            <Link href={route('ppns.edit',[ppn.id])} className='mb-2 me-2 inline-flex items-center rounded-lg bg-gradient-to-r from-lime-200 via-lime-400 to-lime-500 px-5 py-2.5 text-center text-sm font-medium text-gray-900 shadow-lg shadow-lime-500/50 hover:bg-gradient-to-br focus:outline-none focus:ring-4 focus:ring-lime-300 dark:shadow-lg dark:shadow-lime-800/80 dark:focus:ring-lime-800'>
                                                <Icon icon={'IconLogin2'} /> Edit
                                            </Link>
                                            <Link as='button' method='delete' href={route('ppns.destroy',[ppn.id])} className='mb-2 mb-2 me-2 me-2 inline-flex rounded-lg bg-gradient-to-r from-red-400 via-red-500 to-red-600 px-5 py-2.5 text-center text-sm font-medium text-white shadow-lg shadow-red-500/50 hover:bg-gradient-to-br focus:outline-none focus:ring-4 focus:ring-red-300 dark:shadow-lg dark:shadow-red-800/80 dark:focus:ring-red-800'>
...
Index.layout = (page) => <AppLayout title={'PPN'} children={page} />;
